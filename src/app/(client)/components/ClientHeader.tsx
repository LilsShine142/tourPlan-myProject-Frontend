...
export default function ClientHeader() {
  // Giả lập state auth. Trong thực tế bạn sẽ lấy từ Context/Zustand hoặc NextAuth
  const isLoggedIn = true; 

  // Menu dropdown cho người dùng đã đăng nhập ở trang ngoài
  const userMenu: MenuProps['items'] = [
    { key: '1', label: <Link href="/profile">Hồ sơ của tôi</Link> },
    { key: '2', label: <Link href="/trips">Quản lý chuyến đi</Link> },
    { type: 'divider' },
    { key: '3', label: <span className="text-red-500">Đăng xuất</span> },
  ];
  ...
  return (
    <header className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-gray-100 shadow-sm">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        {/* Navigation - Giữ nguyên cho cả 2 trạng thái */}
        ...
        <div className="flex items-center gap-3">
          {!isLoggedIn ? (
            ...
            <>
              <Link href="/main" className="hidden sm:block">
                <Button type="default" className="rounded-xl font-semibold border-gray-200 hover:border-teal-500 hover:text-teal-600">
                  Vào Dashboard
                </Button>
              </Link>
              <Dropdown menu={{ items: userMenu }} placement="bottomRight" trigger={['click']}>
                <Avatar size="large" icon={<UserOutlined />} className="cursor-pointer bg-teal-100 text-teal-600 border border-teal-200" />
              </Dropdown>
            </>
          )}
        </div>
